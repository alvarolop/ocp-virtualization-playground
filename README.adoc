...
If you installed your cluster using the `ocp-on-aws` repository, the easiest way is to link the configuration files from both repos, as they are always equivalent:

[source, bash]
----
ln -s $FILENAME_FROM_INSTALL_REPO aws-env-vars
----


=== Creating the Baremetal instance
...
Take into account that pricing for Bare Metal nodes is considerably higher than for normal instances. For example, `c5.metal` costs $4.4/hour while `m7.xlarge` is just $0.222. https://aws.amazon.com/ec2/pricing/on-demand/[Here] you can find a full list of prices.
====

For more information, check the https://docs.openshift.com/container-platform/4.17/machine_management/creating_machinesets/creating-machineset-aws.html#machineset-yaml-aws_creating-machineset-aws[official documentation].
...
Red Hat OpenShift Virtualization is a Kubernetes operator on RHOCP, based on the Kubernetes virtualization add-on, KubeVirt, which allows the management of VM workloads alongside container workloads. The following ArgoCD application wil configure the following:

1. OCP-Virt operator.
2. Create the `HyperConverged` object, which deploys and handles OCP-Virt and its components.
3. NMstate operator and default configuration.
4. Basic network configuration for the VMs.

[source, bash]
----
oc apply -f application-ocp-virt.yaml
----
...
[source, bash]
----
oc process -f docs/ocp-tools/01-toolbox.yaml -p NAMESPACE=ocp-virt-pgd | oc apply -f -
----
